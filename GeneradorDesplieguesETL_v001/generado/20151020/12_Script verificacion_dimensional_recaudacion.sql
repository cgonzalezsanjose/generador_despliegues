...
IF EXISTS (SELECT 1 FROM dbn1_stg_dhyf.INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='dbo' AND ROUTINE_NAME='spn1_verificacion_dimensional_recaudacion' AND ROUTINE_TYPE='PROCEDURE')
...
BEGIN
  ...
  DECLARE @procedure_name varchar(100)='dbn1_stg_dhyf.dbo.spn1_verificacion_dimensional_recaudacion'
  SET NOCOUNT ON;
  DECLARE @log int;
    ...
    SELECT 1 FROM (
      ...
      FROM
        (SELECT
         ...
         FROM dbn1_dmr_dhyf.dbo.tbn1_fact_ot_operaciones_contables f
         INNER JOIN dbn1_dmr_dhyf.dbo.tbn1_dim_ot_deudas d ON (d.id_deuda=f.id_deuda)
  ...
  IF NOT EXISTS (
    SELECT 1 FROM (
      SELECT
        anyo,
        count(8) AS num_exp
      FROM dbn1_dmr_dhyf.dbo.tbn1_dim_ot_z3_expedientes_embargo
      WHERE
  ...
  IF NOT EXISTS (
    SELECT 1 FROM (
      SELECT
        anyo,
        count(8) AS num_exp
      FROM dbn1_dmr_dhyf.dbo.tbn1_dim_ot_sz_expedientes_aplazamiento
      WHERE
  ...
  IF NOT EXISTS (
    SELECT 1 FROM (
      SELECT
        ...
        count(8) AS num_exp
      FROM dbn1_dmr_dhyf.dbo.tbn1_dim_ot_av_expedientes_fianza
      WHERE
  ...
  IF NOT EXISTS (
    SELECT 1 FROM (
      SELECT
        anyo_expediente,
        count(8) AS num_exp
      FROM dbn1_dmr_dhyf.dbo.tbn1_dim_ot_z6_expedientes_concursales
      WHERE
  ...
  IF NOT EXISTS (
    SELECT 1 FROM (
      SELECT
        anyo,
        count(8) AS num_exp
      FROM dbn1_dmr_dhyf.dbo.tbn1_dim_ot_dz_expedientes_derivacion
      WHERE
  ...
  IF EXISTS (SELECT 1 FROM dbn1_norm_dhyf.INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='audit' AND TABLE_NAME='tbn1_test_cuadre_s1' AND TABLE_TYPE='BASE TABLE')
  DROP TABLE dbn1_norm_dhyf.audit.tbn1_test_cuadre_s1
  ...
  INTO dbn1_norm_dhyf.audit.tbn1_test_cuadre_s1
  FROM
    (SELECT
     ...
     FROM
         ...
         FROM dbn1_dmr_dhyf.dbo.tbn1_fact_ot_operaciones_contables f
         INNER JOIN dbn1_dmr_dhyf.dbo.tbn1_dim_ot_deudas d ON (d.id_deuda=f.id_deuda)
         INNER JOIN dbn1_dmr_dhyf.dbo.tbn1_dim_ot_deudas_perfil1 perfil ON (perfil.id_perfil_deuda=d.id_perfil_deuda)
         GROUP BY
           d.num_liquidacion,
           perfil.ejercicio,
           perfil.cod_modelo,
  ...
  -- IF NOT EXISTS (
    -- SELECT * FROM (
      ...
      -- FROM dbn1_dmr_dhyf.dbo.tbn1_fact_ot_operaciones_contables
      -- WHERE des_clave_contable='Clave contable faltante en Maestro'
    -- ) a WHERE cont=0
  -- )
  -- EXECUTE dbn1_stg_dhyf.dbo.spn1_apuntar_warning @logpadre,'Existen claves contables asociadas a movimientos, que no figuran en el maestro';
  ...
  -- IF NOT EXISTS (
    ...
    -- ) a WHERE cont=0
  -- )
  -- EXECUTE dbn1_stg_dhyf.dbo.spn1_apuntar_warning @logpadre,'Existen códigos de operación asociados a movimientos, que no figuran en el maestro';
